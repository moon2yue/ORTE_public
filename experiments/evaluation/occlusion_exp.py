...
def main(args):
    ...
    if Dname == 'epilepsy':
        ...
        trainB, val, test = process_Boiler_OLD(split_no = args.split_no, device = device, base_path = '')
        trainX = trainB[0]
    else:
    ...
    if args.exp_path is not None:
        ...
        if args.exp_method == 'ours' or args.exp_method == "timex" or args.exp_method == "orta":
    ...
    for upper_pct in tlist:
        ...
        with torch.no_grad():
            if args.exp_method == 'timex' or args.exp_method == "timex++" or args.exp_method == "orta":
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_method', type = str, help = "Options: ['random', 'ig', 'dyna', 'winit', 'timex', 'timex++', 'orta']", default='orta')
    parser.add_argument('--dataset', type = str, default='epilepsy',help = "Options: ['boiler', 'epilepsy', 'pam']")
    ...
    for xxx in range(1, 6):
        fold = xxx
        args.split_no = xxx
        if is_timex and args.exp_method=='ours':
            args.exp_method = "timex"
        add_str = "bc"
        if not is_timex:
            add_str = "our_bc"

        if args.exp_method=='timex' or args.exp_method=='timex++':
            if args.dataset=="pam":
                args.model_path = "".format(add_str,fold)
            elif args.dataset=="epilepsy":
                args.model_path = "".format(add_str,fold)
            elif args.dataset=="boiler":
                args.model_path = "".format(add_str,fold)
        elif args.exp_method=='orta':
            if args.dataset=="pam":
                args.model_path = "".format(fold)
            elif args.dataset=="epilepsy":
                args.model_path = "".format(fold)
            elif args.dataset=="boiler":
                args.model_path = "".format(fold)
        ...
        if args.exp_method == 'random':
            ...
            average_df = combined_df.groupby(combined_df.thresh).mean() 
            tlist = [0.75, 0.8, 0.85, 0.9, 0.925, 0.95, 0.975, 0.99]
            average_df["thresh"] = tlist

            savepath = ''.format(args.dataset, args.exp_method, xxx)
            savepath = os.path.join(args.savedir, savepath)
            print('Saving at {}'.format(savepath))
            average_df.to_csv(savepath, index = False)
