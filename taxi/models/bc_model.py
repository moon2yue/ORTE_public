...
class TimeXModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.mask_generator = MaskGenerator(d_z = (self.d_inp + self.d_pe),
                                            d_pe = self.d_pe, max_len = max_len,
                                            tau = self.tau,
                                            tau_backward = tau_backward,
                                            use_ste = self.ablation_parameters.use_ste,
                                            use_decoupled_ste=decoupled_ste)
    ...
    def forward(self, src, times, captum_input = False):
        ...
        exp_src, reference_src, reference_src_rest = self.multivariate_mask(src, ste_mask)

        if self.ablation_parameters.archtype == 'transformer':
            pred_mask, z_mask, z_seq_mask = self.encoder_main(exp_src, times, get_agg_embed = True)
            pred_mask_rest, z_mask_rest, z_seq_mask_rest = self.encoder_main(reference_src_rest, times, get_agg_embed=True)
        else:
