...
default_abl = AblationParameters()
...
class TimeXModel(nn.Module):
    ...
    def __init__(self,
        ...
        if self.ablation_parameters.archtype == 'transformer':
            ...
            self.encoder_main_univariate = TransformerMVTS(
                d_inp=1,  # Dimension of input from samples (must be constant)
                max_len=max_len,  # Max length of any sample to be fed into model
                n_classes=self.n_classes,  # Number of classes for classification head
                **self.transformer_args
            )
        ...
        self.encoder_pret_univariate = TransformerMVTS(
            d_inp=1,  # Dimension of input from samples (must be constant)
            max_len=max_len,  # Max length of any sample to be fed into model
            n_classes=self.n_classes,  # Number of classes for classification head
            **self.transformer_args
        ...
        self.mask_generator = MaskGenerator(d_z=(self.d_inp + self.d_pe),
                                            d_pe=self.d_pe,
                                            max_len=max_len,
                                            tau=self.tau,
                                            use_ste=self.ablation_parameters.use_ste,

                                            use_decoupled_ste = use_decoupled_ste,
                                            forward_ste_temperature = forward_ste_temperature,
                                            backward_ste_temperature = backward_ste_temperature,
                                            ste_learning_rate = ste_learning_rate,
                                            )

        self.mask_connection_src = MLP([2, 32, 1], activations='elu', dropout=0.0)
        self.mask_connection_src_v2 = MLP([self.d_inp, 32, self.d_inp], activations='elu', dropout=0.0)
    ...
    def forward(self, src, times, captum_input=False):
        ...
        # self.ablation_parameters.g_pret_equals_g = True
        ...
        exp_src, reference_src, reference_src_rest = self.multivariate_mask(src, ste_mask, self.noise_level)

        if self.ablation_parameters.archtype == 'transformer':
            pred_mask, z_mask, z_seq_mask = self.encoder_main(exp_src, times, get_agg_embed=True)
            pred_mask_rest, z_mask_rest, z_seq_mask_rest = self.encoder_main(reference_src_rest, times,
                                                                             get_agg_embed=True)
        else:
            pred_mask, z_mask = self.encoder_main(exp_src, times, get_embedding=True)
            pred_mask_rest, z_mask_rest = self.encoder_main(reference_src_rest, times, get_embedding=True)
        ...
        total_out_dict = {
            'pred': pred_regular,
            # Prediction on regular embedding (prediction branch) #
            'pred_mask': pred_mask,  # Prediction on masked embedding #
            'mask_logits': mask_in,  # Mask logits, i.e. before reparameterization + ste #
            'ste_mask': ste_mask,  #
            'smooth_src': src,  #
            'all_z': (src_distr, mask_src_distr),
            'reference_z': (reference_src, exp_src, reference_src_rest),
            'vis': (z_main, z_mask, z_mask_rest),
        }
    ...
    def multivariate_mask(self, src, ste_mask, noise_level):
        # First apply mask directly on input:
        baseline = noise_level*self._get_baseline(B=src.shape[1])
        ...
        src_masked_ref_rest = src * (1 - ste_mask_rs) + ste_mask_rs * baseline

        src_masked = self.mask_connection_src(torch.stack([src, ste_mask_rs], dim=-1)).squeeze(-1)
        return src_masked, src_masked_ref, src_masked_ref_rest
