...
@torch.no_grad()
def eval_mv4_loader(test_loader, model, gt_exps=None, masked = False):
    # Also evaluates models above v4
    model.eval()
    out = {}
    y = []
    ...
    out_pred = []
    out_pred_mask = []
    for X, times_dict, y_dict, ids in test_loader:  # Need negative sampling here
        out_dict = model(X.transpose(0,1), times_dict.transpose(0,1), captum_input=False)
        out_mask_logits.append(out_dict['mask_logits'])
        out_pred.append(out_dict['pred'])
        out_pred_mask.append(out_dict['pred_mask'])
        y.append(y_dict)

    out_mask_logits_all = torch.cat(out_mask_logits,dim=0)
    out_pred_all = torch.cat(out_pred, dim=0)
    out_pred_mask_all = torch.cat(out_pred_mask, dim=0)
    y = torch.cat(y, dim=0)

    out['mask_logits'] = out_mask_logits_all
    out['pred'] = out_pred_all
    out['pred_mask'] = out_pred_mask_all
